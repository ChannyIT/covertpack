...
ensure_geyser_mapping_from_script () {
  ...
  local mapping_version="${GEYSER_MAPPING_VERSION:-2}"
  if [[ -f "geyser_mappings_v2.json" && "${mapping_version}" != "1" ]]; then
    cp -f "geyser_mappings_v2.json" ./target/geyser_mappings.json
    status_message completion "Using pre-generated Geyser mappings v2"
    return
  elif [[ ! -f ./target/geyser_mappings.json ]]; then
    jq -n '{"format_version":2,"items":{}}' | sponge ./target/geyser_mappings.json
  fi
  ...
  jq -s '
    ...
    | .[1] as $extra_entries
    | .[2] as $base_mappings
    | (($base_mappings.format_version // 2 | tostring) == "2") as $is_v2
    | ($base_mappings.items // {}) as $base_items
    | ($extra_entries | sort_by(.item) | group_by(.item) | map({(.[0].item): map(.mapping + (if $is_v2 then {"type":"legacy","display_name":.item} else {} end))}) | add // {}) as $extra_items
    | {
        "format_version": (if $is_v2 then 2 else ($base_mappings.format_version // "1") end),
        "items": (
...
collect_conversion_diagnostics () {
  ...
  for diagnostics_file in plugin_context.json gui_map.json rank_map.json particle_map.json entity_map.json block_map.json resource_map.json coverage_report.json; do
...
echo
if [[ -f "geyser_mappings_v2.json" && "${GEYSER_MAPPING_VERSION:-2}" != "1" ]]; then
  cp -f "geyser_mappings_v2.json" ./target/geyser_mappings.json
else
  jq '
([map(
...
if [[ -f "geyser_mappings_v2.json" && "${GEYSER_MAPPING_VERSION:-2}" != "1" ]]; then
  status_message completion "Using pre-generated Geyser mappings v2; skipping legacy sprite merge"
else
