...
def _merge_state_maps(
    base_maps: List[Dict[str, str]], branch_maps: List[Dict[str, str]]
) -> List[Dict[str, str]]:
    if not base_maps:
...
def _state_maps_from_when(when: object) -> List[Dict[str, str]]:
    ...
    if isinstance(when, str):
        ...
        or_chunks = [
            chunk.strip()
            for chunk in re.split(r"\|\||\bor\b", raw, flags=re.IGNORECASE)
            if chunk.strip()
        ]
        if len(or_chunks) > 1:
            out = []
...
def _expand_variant_state_key(state_key: object) -> List[str]:
    ...
    for part in raw.split(","):
        ...
        raw_values = [
            chunk.strip()
            for chunk in re.split(r"\|\||\bor\b|\|", value, flags=re.IGNORECASE)
            if chunk.strip()
        ]
        if not raw_values:
    ...
    for group in option_groups:
        ...
        for prefix in combinations:
            for item in group:
                if not item:
                    continue
                next_combinations.append(f"{prefix},{item}" if prefix else item)
        combinations = next_combinations

    return sorted(set(combinations)) if combinations else [raw]


# ──────────────────────────────────────────────
# Blockstate model iteration
# ──────────────────────────────────────────────
...
def _iter_state_models(data: Dict[str, object]) -> Iterable[Tuple[str, str]]:
    ...
    if isinstance(variants, dict):
        for state_key, variant_value in variants.items():
            for normalized_key in _expand_variant_state_key(state_key):
    ...
    if isinstance(multipart, list):
        for part in multipart:
            ...
            if not models:
                continue
            for state_key in _state_key_from_when(when):
                for model in models:
...
# ──────────────────────────────────────────────
# Tripwire state normalisation
# ──────────────────────────────────────────────

# Tripwire blockstate properties (alphabetical in 1.20+):
#   attached, disarmed, east, north, powered, south, west
# The JSON key order can vary; we sort to a canonical order.
_TRIPWIRE_PROPERTY_ORDER = [
    "attached", "disarmed", "east", "north", "powered", "south", "west"
]


def _normalize_tripwire_state(state_key: str) -> str:
    """Reorder tripwire state-key parts to a stable canonical order.

    BUG FIXED: original hard-coded index reordering assumed a specific
    incoming order which differed between MC versions and broke easily.
    Now parses key=value pairs and re-sorts by known property order.
    """
    if "=" not in state_key:
        return state_key
    ...
    # Build ordered list: known properties first, then unknown alphabetically
    ordered: List[str] = []
    for prop in _TRIPWIRE_PROPERTY_ORDER:
        if prop in pairs:
            ordered.append(f"{prop}={pairs[prop]}")
    for prop in sorted(pairs.keys()):
        if prop not in _TRIPWIRE_PROPERTY_ORDER:
            ordered.append(f"{prop}={pairs[prop]}")

    return ",".join(ordered) if ordered else state_key
...
# ──────────────────────────────────────────────
# Model-ref normalisation
# ──────────────────────────────────────────────

def _normalize_model_ref(model_ref: str, block_namespace: str) -> str:
    """Ensure the model ref has a namespace prefix.

    BUG FIXED: blockstates sometimes emit refs like 'minecraft:block/stone'
    when the pack namespace is 'minecraft'.  blocks_util.get_am_file splits
    on ':' correctly, so this is mostly a safety guard for refs without any
    namespace.
    """
    ...
    if not model_ref:
        return model_ref
    if ":" not in model_ref:
        # No namespace – inherit from the blockstate file's namespace
        return f"{block_namespace}:{model_ref}"
    return model_ref
...
def run() -> None:
    ...
    # Write shared Bedrock assets
    blocks_util.write_animated_cube()
    blocks_util.write_geometry_cube()
    # FIX: initialise terrain_texture.json BEFORE the first call to
    # create_terrain_texture() – the original code never did this, causing
    # an immediate FileNotFoundError crash for every block.
    blocks_util.write_terrain_texture()
    ...
    for namespace, blockstate_file in blockstate_files:
        ...
        # Skip fire (Bedrock handles it natively)
        ...
        try:
            with blockstate_file.open("r", encoding="utf-8") as f:
                data = json.load(f)
        except Exception as exc:
            _log(f"Failed to parse {blockstate_file}: {exc}")
            parse_failures.append(
                f"{blockstate_file.relative_to(PACK_DIR).as_posix()}: {exc}"
            )
            continue
        ...
        for state_key, model_ref in _iter_state_models(data):
            ...
            # Skip known-problematic vanilla refs
            if "block/original" in model_ref or "block/tripwire_attached_n" in model_ref:
                continue

            # FIX: ensure namespace is present before lookup
    ...
    payload = {
        ...
        "unresolved_ref_count": (
            len(unresolved_model_refs)
            + len(unresolved_sources)
            + len(parse_failures)
        ),
        "missing_ref_count": (
            len(unresolved_model_refs)
    ...
    _log(f"Converted {converted} block variants across {len(converted_blocks)} blocks")
